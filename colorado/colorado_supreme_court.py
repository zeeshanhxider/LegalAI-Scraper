...
COURT_NAME = "colorado_supreme_court"
...
ROW_SELECTOR_CANDIDATES = [
    ROW_SEL,
    "li.documento__result-item.document",
    "li.document",
]
...
def extract_year_value(date_text: str, docket_number: str = "") -> str:
    ...
    # Fallback: some dockets include year at the end (e.g., 25SC123).
...
async def wait_for_detail_page_ready(detail_page):
    # The Colorado site is a JS app and sometimes paints metadata late.
    # Poll briefly until either docket text exists or the page looks fully rendered.
    for _ in range(18):
        has_docket = await detail_page.evaluate(
            "() => !!(document.body && document.body.innerText.includes('Docket Number'))"
        )
        body_len = await detail_page.evaluate(
            "() => (document.body && document.body.innerText ? document.body.innerText.trim().length : 0)"
        )
        has_download_ui = await detail_page.locator("div#formats, [aria-label='Download']").count()

        if has_docket or (body_len > 500 and has_download_ui > 0):
            return
        await detail_page.wait_for_timeout(1000)

async def wait_for_search_results(search_page):
    last_error = None
    for attempt in range(1, 5):
        try:
            for sel in ROW_SELECTOR_CANDIDATES:
                try:
                    await search_page.wait_for_selector(sel, state="attached", timeout=30000)
                    if await search_page.locator(sel).count() > 0:
                        return sel
            ...
            if "Documents (" in body_text:
                # If the page clearly loaded results text, use the default selector.
                return ROW_SEL
        except Exception as e:
            last_error = e

        if attempt < 4:
            await search_page.reload(wait_until="domcontentloaded", timeout=120000)
            await search_page.wait_for_timeout(1500)

    raise RuntimeError(f"Search results failed to load after retries: {last_error}")
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        active_row_sel = await wait_for_search_results(search_page)
        ...
        with open(CSV_PATH, "w", newline="", encoding="utf-8") as f:
            ...
            while True:
                rows_loc = search_page.locator(active_row_sel)
                ...
                new_count = await search_page.locator(active_row_sel).count()
                if new_count <= count:

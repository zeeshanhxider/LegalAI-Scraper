...
DEFAULT_START_TERM = "2003-01"
DEFAULT_TERM = datetime.now().strftime("%Y-%m")
BASE_URL = (
    "https://www.sccourts.org/opinions-orders/opinions/unpublished-opinions/"
    "court-of-appeals/?term={term}"
)
HEADLESS = True
...
def ensure_folders(base_dir: Path) -> Dict[str, Path]:
    downloads_dir = base_dir / "downloads/unpublished_court_of_appeals"
    csv_dir = downloads_dir / "CSV"
    pdf_dir = downloads_dir / "PDF"
    log_dir = base_dir / "Log/unpublished_court_of_appeals"
...
def _normalize_case_no(raw_case_no: str) -> str:
    ...
    text = re.sub(r"^(case\s*(no\.?|number)?\s*[:#-]?\s*)", "", text, flags=re.IGNORECASE).strip()

    formatted_match = re.search(r"[A-Za-z0-9]+(?:-[A-Za-z0-9]+)+", text)
    if formatted_match:
        return formatted_match.group(0)
...
def _page_has_no_records_message(driver: webdriver.Chrome) -> bool:
    ...
    return (
        "no unpublished opinions for this month" in source
        or "there are no unpublished opinions" in source
    )
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Scrape South Carolina Court Of Appeals unpublished opinions.")

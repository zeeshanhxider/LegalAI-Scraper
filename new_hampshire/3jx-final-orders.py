...
def build_year_url(year: int) -> str:
    return f"{BASE_URL}/our-courts/supreme-court/orders-and-opinions/3jx-final-orders/{year}"
...
def setup_logger() -> logging.Logger:
    ...
    logger = logging.getLogger("nh_3jx_final_orders_scraper")
    logger.setLevel(logging.INFO)
    logger.handlers.clear()

    log_name = f"nh_supreme_court_3jx_final_orders-{datetime.now().strftime('%Y-%m-%d')}.log"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="NH Supreme Court 3JX Final Orders scraper")

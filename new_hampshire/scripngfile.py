...
def get_current_page_number(driver: webdriver.Chrome) -> Optional[str]:
    try:
        ...
        return active.get_attribute("data-page") or active.text.strip()
...
def click_next_page(driver: webdriver.Chrome, logger: logging.Logger, year: int) -> bool:
    try:
        next_btn = driver.find_element(
            By.CSS_SELECTOR,
            ".document__list__pagination .tabulator-page[data-page='next']",
        )
    except NoSuchElementException:
        logger.info("Year %s | Next button not found. Pagination finished.", year)
        return False
    ...
    try:
        ...
        try:
            ...
            logger.info("Year %s | Could not click next page button.", year)
            return False
    ...
    try:
        ...
        logger.info("Year %s | Page number did not change after clicking next.", year)
        return False
...
def scrape_all_pages_for_year(driver: webdriver.Chrome, year: int, logger: logging.Logger) -> List[Dict[str, str]]:
    ...
    rows_found = wait_for_rows(driver, timeout=20)
    if not rows_found and page_has_no_results(driver):
...
def main() -> None:
    ...
    try:
        ...
        driver = get_driver(headless=bool(args.headless))
